...
### Сравнение инструментов мониторинга и алертинга

| Критерий                                             | Grafana (выбран)                                                                  | Datadog                                                                                                                 | Prometheus + Alertmanager                                                                                             |
|------------------------------------------------------|-----------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------|
| Лицензия и стоимость владения                        | AGPL v3, open source. Бесплатно для self-hosted.                                  | Проприетарная. Платная — стоимость зависит от объёма данных и числа хостов, быстро растёт.                              | Apache 2.0, полностью бесплатно.                                                                                      |
| Производительность и масштабируемость                | Горизонтально масштабируется. Grafana Cloud доступна как SaaS.                    | Полностью управляемый SaaS, высокая производительность из коробки.                                                      | Отлично масштабируется для метрик, но не предназначен для работы с трейсами напрямую.                                 |
| Функциональность (визуализация, алертинг, аналитика) | Богатые дашборды, алертинг, TraceQL для запросов к трейсам, поддержка RED-метрик. | Очень широкая функциональность: APM, трейсинг, логи, метрики в одном инструменте.                                       | Мощный алертинг по метрикам, но визуализация трейсов отсутствует — нужна Grafana поверх.                              |
| Простота внедрения и поддержки                       | Умеренная сложность. Хорошая документация, большое сообщество.                    | Минимальная — SaaS, агент устанавливается за несколько минут.                                                           | Умеренная. Хорошо знаком команде (уже выбран в задании 2).                                                            |
| Совместимость с Jaeger                               | Нативный плагин Jaeger Data Source — запросы к трейсам прямо из Grafana.          | Собственный APM, не использует Jaeger UI. Интеграция через OpenTelemetry, но трейсы попадают в Datadog, а не в Jaeger.  | Нет прямой интеграции. Трейсы нужно конвертировать в метрики через SpanMetrics, только потом Prometheus их собирает.  |

### Обоснование выбора Grafana

- Нативная интеграция с Jaeger — Grafana подключается к Jaeger Query напрямую через встроенный плагин. Не нужно конвертировать трейсы в метрики, как в случае с Prometheus.
- Единый инструмент для трейсов и алертинга — Grafana одновременно визуализирует трейсы из Jaeger и строит алерты. Datadog предлагает то же самое, но за значительные деньги.
- Стоимость — Grafana OSS бесплатна. Datadog при росте нагрузки становится дорогим быстро.
- Prometheus уже в стеке — Grafana умеет работать и с Prometheus (задание 2), и с Jaeger одновременно. Это позволяет иметь единый дашборд для метрик и трейсов без дополнительных компонентов.

### Минусы, которые приняты

- Настройка алертинга на основе трейсов сложнее, чем на основе метрик.

**Итог**: Grafana — наиболее сбалансированное решение: бесплатная, нативно интегрируется с Jaeger и закрывает все потребности — визуализацию трейсов и алертинг — без дополнительных компонентов.
...
| Медленный ответ Shop API | P95 времени ответа превысил 500 мс    | Команда разработки | HIGH       |
| Медленный ответ MES API  | P95 времени ответа превысил 500 мс    | Команда разработки | HIGH       |
| Медленный ответ CRM API  | P95 времени ответа превысил 500 мс    | Команда разработки | HIGH       |
